...
export default {
  ...
  methods: {
    ...
    onShortcutTouchMove(e) {
      ...
      let delta = ((this.touch.y2 - this.touch.y1) / ANCHOT_HEIGHT) | 0; //获取y轴的偏移，获取目前的元素
    ...
    _scrollTo(index) {
      //list-shortcut的头尾加了2个li美化ui, 点击这2个li时不作处理
      if (!index && index !== 0) {
        return;
      }
      //处理index的边界值
    ...
    _calculateHeight() {
      ...
      let height = 0;
      //这里push多一次，所以scrollY中间部分滚动要length -1
      this.listHeight.push(height);
      for (let i = 0; i < list.length; i++) {
        let item = list[i];
        // item是dom，可以直接获取clientHeight
